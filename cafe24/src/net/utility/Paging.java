...
public String paging4(int totalRecord, int nowPage, int recordPerPage, String col, String word, String filenm){
    int pagePerBlock = 10; // 블럭당 페이지 수
    int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지
    int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹
    int nowGrp = (int)(Math.ceil((double)nowPage/pagePerBlock));    // 현재 그룹
    ...
    for(int i=startPage; i<=endPage; i++){
      ...
      if (nowPage == i){
    	str.append("<LI>");
        str.append("<A href='./"+filenm+"?col="+col+"&word="+word+"&nowPage="+i+"' style='font-weight: bold; color:black;'>&nbsp;"+i+"&nbsp;</A>");
        str.append("</LI>");
      }else{
    	str.append("<LI>");  
        str.append("<A href='./"+filenm+"?col="+col+"&word="+word+"&nowPage="+i+"'>&nbsp;"+i+"&nbsp;</A>&nbsp;");
