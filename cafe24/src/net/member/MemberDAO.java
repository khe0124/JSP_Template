...
public class MemberDAO {
	...
	// 8) ID 찾기
	public MemberDTO faproc(String mname, String email) {
		MemberDTO dto = null;
		try {
			// 1. DB연결: DBOpen.java와 연결한다.
			Connection con = DBOpen.getConnetion();
			...
			// 3. SQL문 변환
			PreparedStatement pstmt = con.prepareStatement(sql.toString());
			pstmt.setString(1, mname); // ?순서와 ?에 들어갈 자료형 주의
			pstmt.setString(2, email); // ?순서와 ?에 들어갈 자료형 주의
	...
	// 9) 계정확인
	public int fpexist(MemberDTO dto) {
		int cnt = 0;
		try {
			// 1. DB연결
			Connection con = DBOpen.getConnetion();
			...
			System.out.println("아이디 확인 실패!: " + e);
		}
		return cnt;
	}

	// 10) 새로운 비밀번호로 수정(비밀번호찾기)
	public int fpUpdate(String id, String passwd) {
		int cnt = 0;
		try {
			// 1. DB연결: DBOpen.java와 연결한다.
			Connection con = DBOpen.getConnetion();
	...
	// 11) 회원정보 조회
	public MemberDTO fpproc(String id) {
		MemberDTO dto = null;
		try {
			// 1. DB연결: DBOpen.java와 연결한다.
			Connection con = DBOpen.getConnetion();
	...
	// 12) 권한관리
	public MemberDTO userLevel(String id) {
		MemberDTO mdto = null;
		...
		try {
			...
			if(rs.next()){
				do {
					mdto = new MemberDTO(); //한 줄씩 저장하기
					mdto.setMlevel(rs.getString("mlevel"));
				} while (rs.next()); 
